...
const run = async (): Promise<void> => {
  const dbUrl = (process.env.DB_URI as string) || 'postgres://postgres:postgres@localhost:5432/postgres';
...
const importHaruraMetadata = () => {
  const url = (process.env.HASURA_URL as string) || 'http://localhost:8080/v1/query';
